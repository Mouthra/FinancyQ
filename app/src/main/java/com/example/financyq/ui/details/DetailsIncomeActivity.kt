...
import com.example.financyq.databinding.BottomSheetDetailIncomeBinding
...
class DetailsIncomeActivity : AppCompatActivity() {
    ...
    // ViewModels
    private val vmDetail   by viewModels<DetailsIncomeViewModel> { ViewModelFactory.getInstance(this) }
    private val vmUpdate   by viewModels<UpdateIncomeViewModel> { ViewModelFactory.getInstance(this) }
    private val vmDelete   by viewModels<DeleteIncomeViewModel> { ViewModelFactory.getInstance(this) }

    // simpan semua transaksi utk difilter
    private var allTx: List<TransactionsItem> = emptyList()

    // tanggal yang sedang dipilih (0–11, tahun)
    ...
    private fun setupUI() {
        ...
        adapter = DetailsIncomeAdapter { showBottomSheet(it) }
        ...
        binding.btnFilter.setOnClickListener {
            // 1) reset pilihan segmented toggle (harian/mingguan/bulanan)
            binding.periodToggle.clearChecked()
            // 2) panggil kembali applyCurrentFilter() → default case akan menampilkan semua data
            applyCurrentFilter()
        }
    }
    ...
    private fun loadData() {
        ...
        vmDetail.getDetailIncome(uid).observe(this) { res ->
            when (res) {
                is Result.Loading -> {
                    binding.progressBarDetailIncome.visibility = android.view.View.VISIBLE
                }
                is Result.Success -> {
                    binding.progressBarDetailIncome.visibility = android.view.View.GONE
    ...
    private fun applyCurrentFilter() {
        ...
        val list = when (binding.periodToggle.checkedButtonId) {
    ...
    private fun List<TransactionsItem>.filterLast24h(): List<TransactionsItem> {
        val now = Date()
        val cut = Calendar.getInstance().apply { time = now; add(Calendar.DAY_OF_YEAR, -1) }.time
        return filter { it.tanggalString().let { d -> d != null && d.after(cut) } }
    }
    private fun List<TransactionsItem>.filterLast7d(): List<TransactionsItem> {
        val now = Date()
        val cut = Calendar.getInstance().apply { time = now; add(Calendar.DAY_OF_YEAR, -7) }.time
        return filter { it.tanggalString().let { d -> d != null && d.after(cut) } }
    }
    private fun List<TransactionsItem>.filterMonth(year: Int, month: Int): List<TransactionsItem> {
        return filter {
            it.tanggal?.let { ts ->
                ts.substring(0,4).toIntOrNull()==year &&
                        ts.substring(5,7).toIntOrNull()==month+1
            }==true
        }
    }

    // —— perubahan: parse ISO + convert ke Instant → Date
    private fun TransactionsItem.tanggalString(): Date? =
        runCatching {
            val odt = OffsetDateTime.parse(this.tanggal)
            Date.from(Instant.from(odt))
        }.getOrNull()

    // ---- bottom sheet edit / delete ----

    private fun showBottomSheet(item: TransactionsItem) {
        val sheet = BottomSheetDetailIncomeBinding.inflate(LayoutInflater.from(this))
        ...
        sheet.EditNameIncome.setText(item.deskripsi)
        sheet.EditTotalIncome.setText(item.jumlah.toString())
        sheet.EditSource.setText(item.sumber)

        // date picker in sheet
        sheet.EditDate.setOnClickListener {
            ...
            val picker = MaterialDatePicker.Builder
                ...
                .setSelection(  // set default tanggal
                    runCatching {
            ...
            picker.addOnPositiveButtonClickListener { epochMillis ->
                ...
                sheet.EditDate.setText(sdf.format(Date(epochMillis)))
            }
        }
        ...
        sheet.btnDelete.setOnClickListener {
            AlertDialog.Builder(this)
                ...
                .setPositiveButton(R.string.yes) { _, _ ->
                    vmDelete.deleteIncome(item.idTransaksi!!).observe(this) { r ->
        ...
        sheet.btnSave.setOnClickListener {
            ...
            val nameIncome = sheet.EditNameIncome.text.toString().trim()
            val totalStr   = sheet.EditTotalIncome.text.toString().trim()
            ...
            // --- validasi Rp0 ---
            if (totalAmount <= 0) {
                Toast.makeText(this, R.string.total_income_cannot_be_zero, Toast.LENGTH_SHORT).show()
                return@setOnClickListener
            }
            ...
            AlertDialog.Builder(this)
                ...
                .setPositiveButton(R.string.yes) { _, _ ->
                    ...
                    val req = UpdateIncomeRequest(
                        jumlah    = sheet.EditTotalIncome.text.toString().filter { it.isDigit() }.toInt(),
                        sumber    = sheet.EditSource.text.toString(),
                        kategori  = sheet.EditCategory.text.toString(),
                        deskripsi = sheet.EditNameIncome.text.toString(),
                        tanggal   = isoTimestamp       // <-- sekarang ada jam:menit:detik sesuai edit time
                    )
                    vmUpdate.updateIncome(item.idTransaksi!!, req).observe(this) { r ->
                        if (r is Result.Success) {
                            dialog.dismiss()
                            loadData()
                        }
